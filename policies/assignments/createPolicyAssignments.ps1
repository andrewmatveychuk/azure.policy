...
[string[]]$parameterFiles = (Get-ChildItem -Path ".\policies\assignments\" -File -Filter "azuredeploy.parameters.json" -Recurse).FullName
#Removing empty strings from the array if there are any 
$parameterFiles = $parameterFiles | Where-Object { $_ }
...
$parameterFiles.foreach(
    {
        ...
        if ($parameterObject.policyParameters) {
            $rawParameterObject = @{
                ...
                policyParameters     = $parameterObject.policyParameters.value
            }
        }
        else {
            $rawParameterObject = @{
        ...
        if ($resourceGroupName) {
            ...
            New-AzResourceGroupDeployment -ResourceGroupName $resourceGroupName -TemplateFile ".\policies\assignments\azuredeploy.json" -TemplateParameterObject $rawParameterObject -Verbose 
        }
        else {
            #Use subscription-level deployment if policy assignment is not scoped to a specific resource group
            New-AzDeployment -Location $deploymentLocation -TemplateFile ".\policies\assignments\azuredeploy.json" -TemplateParameterObject $rawParameterObject -Verbose
        }
    }
)